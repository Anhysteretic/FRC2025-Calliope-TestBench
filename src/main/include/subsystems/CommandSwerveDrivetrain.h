...
class CommandSwerveDrivetrain : public frc2::SubsystemBase, public TunerSwerveDrivetrain {
    ...
    std::string limelightName = "limelight-main";

    nt::NetworkTableInstance ntInst = nt::NetworkTableInstance::GetDefault();
    std::shared_ptr<nt::NetworkTable> table = ntInst.GetTable("Vision");

    nt::StructPublisher<frc::Pose2d> posePublisherMegaTag1 = table->GetStructTopic<frc::Pose2d>("Megatag 1").Publish();
    nt::StructPublisher<frc::Pose2d> posePublisherMegaTag2 = table->GetStructTopic<frc::Pose2d>("Megatag 2").Publish();
    nt::StructPublisher<frc::Pose2d> targetPublisher = table->GetStructTopic<frc::Pose2d>("target").Publish();
    nt::StructPublisher<frc::Pose2d> robotPose = table->GetStructTopic<frc::Pose2d>("CTRE Pose").Publish();
...
frc2::CommandPtr getGoToScoreCommand(){
    if (!pathplanner::AutoBuilder::isConfigured()){
        configureAutoBuilder();
    }
...
void checkConfigureAutobuilder(){
    if (!pathplanner::AutoBuilder::isConfigured()){
        configureAutoBuilder();
    }
    pathplanner::Pathfinding::ensureInitialized();
