...
void CommandSwerveDrivetrain::Periodic()
{
    ...
    posePublisherMegaTag2.Set(LimelightHelpers::getBotPoseEstimate_wpiBlue_MegaTag2(this->limelightName).pose);
    targetPublisher.Set(pathplannerConstants::target);

    auto PoseEstimate = LimelightHelpers::getBotPoseEstimate_wpiBlue(this->limelightName);
    posePublisherMegaTag1.Set(PoseEstimate.pose);
    if (PoseEstimate.tagCount >= 2){
        this->SetVisionMeasurementStdDevs(std::array<double, 3>{0.7, 0.7, 3.14159*2});
        this->AddVisionMeasurement(PoseEstimate.pose, ctre::phoenix6::utils::GetCurrentTime());
    }
